...
export const fetchDepartments = createAsyncThunk(
  ...
  async ({ page = 1, limit = 5, search = "" }, { rejectWithValue }) => {
    try {
      const response = await ApiService.get(APIURL.DEPARTMENTS, {
        page,
        limit,
        search,
      });
...
export const fetchDepartmentById = createAsyncThunk(
  ...
  async (departmentId, { rejectWithValue }) => {
    try {
      ...
      if (!response.success) {
        return rejectWithValue(response.message || "Failed to fetch department");
      }
...
export const createDepartment = createAsyncThunk(
  ...
  async (payload, { rejectWithValue }) => {
    try {
      ...
      if (!response.success) {
        return rejectWithValue(response.message || "Failed to create department");
      }
...
export const updateDepartment = createAsyncThunk(
  ...
  async ({ id, payload }, { rejectWithValue }) => {
    try {
      ...
      if (!response.success) {
        return rejectWithValue(response.message || "Failed to update department");
      }
...
export const deleteDepartment = createAsyncThunk(
  ...
  async (departmentId, { rejectWithValue }) => {
    try {
      ...
      if (!response.success) {
        return rejectWithValue(response.message || "Failed to delete department");
      }
    ...
    extraReducers: (builder) => {
      ...
      .addCase(deleteDepartment.fulfilled, (state, action) => {
        state.departments = state.departments.filter(
          (dept) => dept.id !== action.payload.departmentId
        );
      });
    }
  })
