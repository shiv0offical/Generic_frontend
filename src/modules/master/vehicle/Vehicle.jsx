...
const Vehicle = () => {
  ...
  const { vehicles, pagination, loading } = useSelector(
    ...
    return info?.map((data, idx) => {
      ...
      return {
        ...
        drivername: data?.driver?.first_name
          ...
          : "-",
        contactNumber: data?.driver?.phone_number
        ...
        status:
          data.status || (data.vehicle_status_id === 1 ? "Active" : "Inactive"),
  ...
  const handleExport = () => {
    // Export whatever rows are visible in the table right now
    // (filtered + sorted + paginated)
    const exportData = vehilceData.map((row) => {
      return {
        "Sl No": row.id,
        "Bus Number": row.busNumber,
        "Route Number": row.routeNumber,
        "Route Name": row.routeName,
        "Driver Name": row.drivername,
        "Contact Number": row.contactNumber,
        "Speed Limit": row.speedLimit,
        "Seat Count": row.seatCount,
        "Bus Name": row.busName,
        Status: row.status,
        "Scanned At": row.createdAt,
  ...
  return (
    <div className="w-full h-full p-2">
      ...
      {isStatusModalOpen && selectedVehicle && (
        <IModal
          toggleModal={isStatusModalOpen}
          onClose={() => setIsStatusModalOpen(false)}
        >
          <div className="p-4">
            <h2 className="text-xl font-semibold mb-4 text-[#07163d]">
              Change Employee Status
            </h2>
            <p className="mb-6">
              Are you sure you want to change status of{" "}
              <strong>{selectedVehicle.busName}</strong> from{" "}
