...
const selectTrackingPanelState = createSelector(
  [
    (s) => s.vehicle.vehicles,
    (s) => s.vehicle.newDevices,
    (s) => s.multiTrackStatus.runningDevices,
    (s) => s.multiTrackStatus.parkedDevices,
    (s) => s.multiTrackStatus.idelDevices,
    (s) => s.multiTrackStatus.devices,
    (s) => s.multiTrackStatus.activeTab,
    (s) => s.multiTrackStatus.offlineVehicleData,
    (s) => s.multiTrackStatus.isTrackShow,
  ],
  (vehicles, newDevices, running, parked, idle, devices, activeTab, offline, isTrackShow) => ({
    vehicles,
    newDevices,
    running,
    parked,
...
const iconStatus = [
  { Icon: FaWifi, key: (d) => ('lat' in d ? d.lat && d.lat !== 0 : null), color: 'green' },
  { Icon: FaKey, key: (d) => ('lat' in d ? d.hasIgnition : null), color: 'green' },
  { Icon: FaBatteryFull, key: (d) => ('lat' in d ? d.hasBattery : null), color: 'green' },
  { Icon: FaBolt, key: (d) => ('lat' in d ? d.hasExternalPower : null), color: 'green' },
];
...
const TrackingPanel = ({ handleRightPanel }) => {
  ...
  const tabCounts = useMemo(
    () => ({
      ...
      All: vehicles?.data ? vehicles.data.length : devices.length,
    }),
    [running.length, idle.length, parked.length, offline.length, newDevices.length, vehicles?.data, devices.length]
  );
        ...
        <div className='flex flex-row'>
          {statusTabs.map((tab) => (
            <button
              ...
              style={{ backgroundColor: tab.bg, borderColor: activeTab === tab.label ? tab.color : 'transparent' }}>
              <div style={{ color: tab.color }}>{tabCounts[tab.label]}</div>
              <div style={{ color: tab.color, fontSize: 11 }}>{tab.label === 'New' ? 'New Device' : tab.label}</div>
            </button>
          ))}
        </div>
        ...
        <div className='mhe-list p-2 mt-2 h-[555px] overflow-y-scroll'>
          {shownDevices.map((device) => (
            <div
              ...
              <div className='h-[10px] w-[10px] rounded-full mr-4' style={{ backgroundColor: device.color }} />
              ...
              {iconStatus.map(({ Icon, key }, i) => {
                ...
                if (status !== null) cls = status ? 'text-[green] text-[15px]' : 'text-red-400 text-[15px]';
                return (
